...
import {motion, AnimatePresence} from 'framer-motion';
 ...
 export const TextRotator = ({texts, interval = 3000, className = ''}: TextRotatorProps) => {
         ...
         <div className={`w-[250px] justify-center relative h-32 flex items-center overflow-hidden ${className}`}>
             <AnimatePresence mode="sync">
                 {[
                     ...
                     {id: `current-${cycle}`, text: texts[index], position: 'current'},
                     {id: `next-${cycle}`, text: texts[nextIndex], position: 'next'}
                 ].map(item => (
                     <motion.div
                         key={item.id}
                         className="absolute"
                     ...
                     >
                         {item.position === 'current' ? (
                             <h2 className="text-2xl font-bold text-blue-600 text-left font-accent">
                                 {item.text}
                             </h2>
                         ) : (
                             <h3 className="text-xl font-semibold text-gray-400 text-left font-accent">
                                 {item.text}
                             </h3>
                         )}
                     </motion.div>
